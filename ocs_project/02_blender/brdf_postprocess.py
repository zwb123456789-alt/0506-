...
def write_png_gamma(path, img_linear, scale=1.0, gamma=2.2):
    ...
    if HAS_CV2:
        ...
        raise RuntimeError("需要 cv2 或 imageio 来写 PNG")
...
def parse_args():
    ...
    p.add_argument("--ggx", action="store_true",
                   help="使用 GGX/Cook-Torrance BRDF（默认 LegacyPhong）")
...
def process(out_dir, save_exr, save_png, ocs_json_override, use_ggx=False):
    out_dir = Path(out_dir)
