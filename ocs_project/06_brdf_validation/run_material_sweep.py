...
"""
import os, sys, json, argparse, time as time_module
import numpy as np
...
def find_latest_plane_batch():
    """自动查找 结果/BRDF验证/ 下最新的 plane_batch_* 目录（按修改时间）。"""
    brdf_dir = PROJECT_ROOT / "结果/BRDF验证"
    candidates = sorted(
        brdf_dir.glob("plane_batch_*"),
...
def parse_args():
    p = argparse.ArgumentParser(description="单平板材料 sweep 三端闭合验证")
    p.add_argument("--exr-dir", default=None,
                   help="plane_batch EXR 目录（默认自动找最新的 plane_batch_*）")
    return p.parse_args()
...
def main():
    ...
    if args.exr_dir:
        ...
        exr_dir = find_latest_plane_batch()
        if exr_dir is None:
            print("[FAIL] 未找到 plane_batch_* 目录，请用 --exr-dir 指定")
            sys.exit(1)
