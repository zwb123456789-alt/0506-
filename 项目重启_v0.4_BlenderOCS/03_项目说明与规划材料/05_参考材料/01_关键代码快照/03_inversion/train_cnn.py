...
_SCAN_GLOB = os.path.join(_PROJECT_ROOT, "结果", "模块A_重构", "multi_geom_ggx_yaw73_pitch37", "run_*", "multi_geom_manifest.json")
_OUT_ROOT_DEFAULT = os.path.join(_PROJECT_ROOT, "结果", "模块C_反演", "cnn_image")
...
def _default_scan_json():
    return _find_latest(_SCAN_GLOB, "multi_geom_manifest.json")
...
def main():
    ...
    ap.add_argument("--out-root", default=None,
                    help="输出根目录，默认 结果/模块C_反演/cnn_image/")
    # 超参
    ...
    # 自动检测默认路径
    if args.scan_json is None:
        args.scan_json = _default_scan_json()
