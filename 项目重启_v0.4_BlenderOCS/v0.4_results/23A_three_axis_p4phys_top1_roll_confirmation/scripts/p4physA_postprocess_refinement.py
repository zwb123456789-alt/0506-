...
PKG23      = V04_ROOT / "v0.4_results" / "23A_three_axis_p4phys_top1_roll_confirmation"
MATRIX_CSV = PKG23 / "tables" / "p4physA_refinement_render_manifest.csv"
...
def process_roll(roll_val, smoke_n=None):
    ...
    print(f"[23A-POST] roll={roll_val:+g}  n={len(units)}")
...
def main():
    ...
    fail  = total - ok
    print(f"\n[23A-POST 全部完成] COMPLETE={ok}/{total}  FAILED={fail}")

    # 写汇总日志
    summ_path = PKG23 / "logs" / "p4physA_postprocess_batches.log"
