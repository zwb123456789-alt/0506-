...
从 23A/tables/p4physA2_render_manifest.csv 读取 render_needed=YES 的点，
...
MATRIX_CSV   = (V04_ROOT / "v0.4_results" / "23B_three_axis_p4phys_pitch_boundary_followup" /
                "tables" / "p4physA2_render_manifest.csv")
OUT_BASE     = (V04_ROOT / "v0.4_results" / "23B_three_axis_p4phys_pitch_boundary_followup" /
...
def main():
    ...
    if smoke_n is not None:
        units = units[:smoke_n]
        print(f"[23B-RENDER SMOKE] n_smoke={smoke_n}")
    if not units:
        print(f"[INFO] roll={roll:+g} 无需渲染（全部复用 P3 或无该 roll 档）。")
        return 0

    print(f"[23B-RENDER] geom=phase63/L1-G1 roll={roll:+g} n_units={len(units)}")
    for u in units[:5]:
