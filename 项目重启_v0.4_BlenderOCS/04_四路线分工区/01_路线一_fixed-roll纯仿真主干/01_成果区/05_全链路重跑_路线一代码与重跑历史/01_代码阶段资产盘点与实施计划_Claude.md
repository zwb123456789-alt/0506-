...
生成时间：2026-06-08
状态更新：2026-06-18
依据：方法冻结通过（`04_四路线分工区/00_总览与裁决/00_路线冻结文件区/04_BlenderOCS方法重建_全局方法冻结文件/13_v0.4前向模型冻结规范_最终冻结版.md` + `04_四路线分工区/00_总览与裁决/00_路线冻结文件区/04_BlenderOCS方法重建_全局方法冻结文件/14_v0.4数据与manifest字段规范_最终冻结版.md`）
...
当前状态：四路线综合审阅与最终路线裁决 R01 已通过；路线一 fixed-roll 主干成立，代码阶段回到 Phase 0 门控验证
```

本文件是路线一代码阶段的资产盘点与实施预案。R01 通过后，本文可作为 `06_v0.4_code/` 骨架搭建、环境记录、单姿态 smoke test 和 Phase 0 gate 的执行参考；实际进入全量生成前仍必须通过 Phase 0 gate。
...
| 旧文件 | 位置 | 可复用内容 | 不可复用内容 |
|---|---|---|---|
| `materials.py` | `03_项目说明与规划材料/05_参考材料/01_关键代码快照/01_code/` | `_GGX_DB` 材料参数字典、GGX 公式结构 | 无（参数已与 `13` §9.3 一致）|
| `geometry.py` | 同上 | STL 加载、姿态旋转矩阵、世界坐标系约定 | 无 |
| `config.py` | 同上 | ortho_scale、resolution、yaw/pitch 网格定义结构 | yaw 网格必须改为 72（不含 360°）|
| `brdf_postprocess.py` | `03_项目说明与规划材料/05_参考材料/01_关键代码快照/02_blender/` | EXR 读取、Normal/Depth/IndexOB 解析结构 | BRDF 计算必须加 `V_sun_macro`；图像响应链必须同步 |
| `inv_common.py` | `03_项目说明与规划材料/05_参考材料/01_关键代码快照/03_inversion/` | per_part_log 特征构建、dataset split 加载结构 | manifest schema 必须升级到 `14` 定义 |
...
这些文件不替代“v0.4 信息量与置信指标实现规范”。指标规范冻结并进入 Phase 0 后，建议在 smoke test 或首次失败时补齐：
