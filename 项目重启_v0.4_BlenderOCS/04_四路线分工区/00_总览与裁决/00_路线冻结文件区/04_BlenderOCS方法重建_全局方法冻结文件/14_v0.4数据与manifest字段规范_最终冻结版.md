# 14 v0.4 数据与 Manifest 字段规范（最终冻结版）

生成时间：2026-06-08
修订基准：Codex 最终复审意见 CR5-001 ～ CR5-006（`12_Codex最终复审意见_前向模型冻结候选.md`）
上版文件：`11_v0.4数据与manifest字段规范_最终冻结候选.md`（已由本最终冻结版取代）

> 本版仅在 `11` 之上做 CR5 小修，CR4-001 ～ CR4-008 已收回内容全部保留。
> CR5 本文件相关修正：①新增"为什么完整统一模型需要更严格 manifest/source_data"立论章节；②image manifest `v_sun_macro_applied` 改为 `v_sun_macro_mode` + `v_sun_macro_applied_to_image`；③字段名 `visibility_level` → `sun_visibility`；④`depth_epsilon_m` 改为记录校准后最终值。
...
## 二、为什么完整统一模型需要更严格的 manifest/source_data，而不是只记录一个 OCS CSV（CR5-001 新增）

### 2.1 立论

只换采样的最小路线只需要一个 OCS CSV：每个姿态一行，记录 yaw/pitch/per-part OCS 即可。但 v0.4 是统一前向模型，OCS、image、fusion、退化实验、multi-geom 扩展全部来自**同一条物理链路的不同读出**。这意味着任何一处的几何、BRDF、可见性、OCS 积分、图像预处理或数据划分发生变化，都会同时影响多个模态的结果。如果只记录一个 OCS CSV，就无法证明某个 image-only / fusion / 退化实验结果到底用了哪一版几何、哪一版 visibility、哪一张 sun shadow mask。

因此 v0.4 必须用结构化 manifest + source_data 把整条链路的可追踪性固化下来。其必要性可从六个维度说明：

| 维度 | 只记录 OCS CSV 的风险 | 严格 manifest/source_data 的保障 |
|---|---|---|
| **geometry 可追踪** | 无法区分不同 STL/姿态网格/ortho_scale/resolution | `geometry_version` + record 级 yaw/pitch/geom_id 唯一定位 |
| **BRDF 可追踪** | 材料参数或 eps 变化无记录 | `brdf_version` 锁定 `_GGX_DB` 与 eps |
| **visibility 可追踪** | 无法证明用了哪一层 sun-side visibility、哪张 shadow mask | `sun_visibility` + `shadow_mapping_method` + `sun_visibility_mask_path` + camera/sun 矩阵 + `depth_epsilon_m` |
| **OCS integration 可追踪** | 像素口径、边缘策略、V_sun_macro 处理无记录 | `ocs_integration_version` + 四类像素统计 |
| **image preprocess 可追踪** | log1p α、I_scale、是否乘 V_sun_macro 无记录 | `image_preprocess_version` + `v_sun_macro_mode` + `I_scale` + `log1p_alpha` |
| **dataset/不可混用** | OCS 与 image 可能来自不同版本而无从发现 | `record_id` 跨 manifest 对齐 + §8 一致性检查 + 禁止 latest-run |

### 2.2 结论

manifest/source_data 不是冗余记录，而是统一前向模型"可追踪、可复审、不可混用"的物理保证。它确保 OCS、image、fusion、退化实验和 multi-geom 扩展在 geometry、BRDF、visibility、OCS integration、image preprocess、dataset split 六个维度上始终对齐，任何一处不一致都能在一致性检查中被发现并拒绝。

---

## 三、Manifest 字段 Schema

### 3.1 OCS Manifest (`ocs_manifest_v0.4.json`) —— CR4-003, CR4-006；CR5-005, CR5-006 修正
...
```json
{
  ...
  "depth_epsilon_m": "<float: 校准后最终值，由 20 姿态 shadow validation 确定；见文件 13 §7.5>",
  "records": [
    {
      ...
      "sun_depth_exr_path": "<string or null: relative to v0.4 data root, null if sun_visibility == \"camera_visible_nol\">",
      "sun_visibility_mask_path": "<string or null: relative to v0.4 data root, null if sun_visibility == \"camera_visible_nol\">",
      "exr_path": "<string>",
      "png_path": "<string>",
      "camera_matrix_world": [
...
**矩阵方向约定**（CR5-002 联动，与文件 13 §7.3.1 一致）：
- `camera_matrix_world`：camera local → world
- `sun_camera_matrix_world`：sun camera local → world
- 派生量 `world_to_camera_matrix = inverse(camera_matrix_world)`、`world_to_sun_camera_matrix = inverse(sun_camera_matrix_world)` 可由代码计算，无需额外存储；但实现必须用正确方向（世界点→相机 local 取逆矩阵）。

**字段说明**（增量标记 CR4/CR5 修正）：
...
| `sun_visibility` | enum | 可见性层级（三选一），一致性检查直接比较此字段 | CR4-003 / CR5-005 |
| `shadow_mapping_method` | string | 实现方式，与 `sun_visibility` 含义不同——前者是"哪种实现"，后者是"哪种 level" | CR4-003 |
| `depth_epsilon_m` | float | sun shadow reprojection 的深度容差（米），**记录校准后最终值** | CR4-003 / CR5-006 |
...
| **`sun_depth_exr_path`** | **string or null** | **sun-view depth EXR 路径，null if `sun_visibility == "camera_visible_nol"`** | **CR4-003 / CR5-005** |
| **`sun_visibility_mask_path`** | **string or null** | **V_sun_macro_mask 路径，null if `sun_visibility == "camera_visible_nol"`** | **CR4-003 / CR5-005** |
| `exr_path` | string | 线性 I_linear EXR 路径 | — |
| `png_path` | string | log1p PNG 路径 | — |
| **`camera_matrix_world`** | **float[4][4]** | **camera-view 相机的世界变换矩阵（camera local → world）** | **CR4-003** |
| **`sun_camera_matrix_world`** | **float[4][4]** | **sun-view 相机的世界变换矩阵（sun camera local → world）** | **CR4-003** |

### 3.2 Image Manifest (`image_manifest_v0.4.json`) —— CR4-001 联动；CR5-004 语义修正
...
```json
{
  ...
  "sun_visibility": "<enum: matches ocs_manifest.sun_visibility>",
  "shadow_mapping_method": "<string: matches ocs_manifest.shadow_mapping_method>",
...
| 字段 | 类型 | 含义 |
|---|---|---|
| `sun_visibility` | enum | 与 OCS manifest 同值，供一致性检查直接比较（CR5-004 新增到 image 侧） |
| `shadow_mapping_method` | string | 与 OCS manifest 同值，供一致性检查直接比较（CR5-004 新增到 image 侧） |
| `preprocessing.v_sun_macro_mode` | enum | `identity`（Level 1，V_sun_macro≡1）或 `shadow_mask`（Level 2，来自 sun shadow reprojection）。取代旧的 `v_sun_macro_applied`（CR5-004） |
| `preprocessing.v_sun_macro_applied_to_image` | bool | 固定 `true`：声明图像线性响应已按 §8.2 乘入 `V_sun_macro`（identity 模式下等价于乘恒等 1） |
| `record_id` | string | 与 OCS manifest 同 key，确保可对齐审计 |

> **CR5-004 修正说明**：旧版 `v_sun_macro_applied == true (iff visibility_version ≥ level 2)` 语义不清——`visibility_version` 是版本号字符串，不应比较"≥ level 2"；且 Level 1 下公式可视为乘了恒等 `V_sun_macro ≡ 1`，"applied" 与否含糊。新版用 `v_sun_macro_mode` 明确区分 `identity` / `shadow_mask`，并用 `v_sun_macro_applied_to_image: true` 声明图像链已按统一公式处理。一致性检查改为比较 `sun_visibility`、`shadow_mapping_method`、`v_sun_macro_mode`（见 §8.1）。
...
| `image_preprocess_version` | log1p α、I_scale、量化参数、PNG 编码方式、v_sun_macro_mode | 任一变化 |
| `dataset_version` | split 策略、train/val/test 划分、seed | 任一分化 |

### 4.2 总 method_version（CR4-008 修正——改为带字段名短格式）
...
**CR5-004 联动**：`image_config.v_sun_macro_mode` 取代旧的 `v_sun_macro_applied`，与 image manifest 同枚举。
...
## 六、run_id / split_id / seed 规则（CR4-007 修正）
...
### 6.2 split_id（CR4-007 修正——删除 seed 具体数字示例）
...
## 八、OCS Source 与 Image Source 一致性规则（CR5-004/005 修正）
...
# V_sun_macro 同源性（CR5-004 修正——不再用 visibility_version ≥ level 2 这种模糊比较）：
...
> **CR5-004/005 修正说明**：旧版用 `image_manifest.v_sun_macro_applied == true (iff visibility_version ≥ level 2)`，把版本号字符串当数值比较，语义含糊。新版改为直接比较 `sun_visibility`、`shadow_mapping_method`、`v_sun_macro_mode` 三个明确字段，杜绝"看起来一致但语义不清"。
...
- ❌ OCS 含 V_sun_macro（shadow_mask）+ 图像 identity（同 visibility level 下 mode 不一致）
...
- ✅ visibility 版本一致（sun_visibility + shadow_mapping_method + v_sun_macro_mode 一致）的 OCS/image 组合

---

## 九、每个 Summary / Figure Source Data 必须记录的字段

### 9.1 Summary 文件 (`summary.json`)
...
## 十、输出目录命名规范（CR4-003 修正——新增 sun shadow / visibility mask 产物）
...
| CR3 编号 | 修正内容 | 本文件对应章节 |
|---|---|---|
| CR3-005 (P1) | 拆分版本字段（6 子版本 + 汇总 method_version） | §4.1, §4.2, §8.1 |
| CR3-008 (P1) | 生成范围分为 single-geom 主线和 multi-geom 扩展两层 | §1.3 |
| CR3-010 (P2) | 输出目录写入 `项目重启_v0.4_BlenderOCS/v0.4_results/` | §10.1, §10.3 |
| CR3-011 (P2) | 删除 split_id 示例中的具体数字 | §6.2, §6.4 |
| CR3-003 (P0) | sun_visibility enum 同步更新为新命名 | §3.1 |

---

## 十二、与 Codex CR4 修正项对应关系

| CR4 编号 | 严重度 | 修正内容 | 本文件对应章节 |
|---|---|---|---|
| CR4-001 | P0 | 图像 preprocess 标注 V_sun_macro 同源（CR5-004 后改为 `v_sun_macro_mode`） | §3.2, §5, §8.1 |
| CR4-002 | P0 | （配套见文件 13）sun shadow 数据流可执行定义 | 前向模型文件 §7 |
| CR4-003 | P0 | manifest 增加 shadow pass 路径、重投影参数、camera/sun 矩阵 | §3.1, §10.2（新增 00b/01 目录） |
| CR4-004 | P1 | （配套见文件 13）GGX eps 规则 | 前向模型文件 §9.2 |
| CR4-005 | P1 | multi-geom 每 geom 都包含 sun shadow pass + BRDF 后处理 | §1.3（第二层清单） |
| CR4-006 | P1 | record_id + 像素统计四类拆分 | §3.1 |
| CR4-007 | P2 | 删除所有 seed 具体数字示例；split_method 写 enum 不写默认例子 | §5（split_config）, §6.2, §6.4 |
| CR4-008 | P2 | method_version 改为带字段名短格式 | §4.2 |

---

## 十三、与 Codex CR5 修正项对应关系（本版新增）

| CR5 编号 | 严重度 | 修正内容 | 本文件对应章节 |
|---|---|---|---|
| CR5-001 | P0 | 新增"为什么完整统一模型需要更严格的 manifest/source_data"独立立论章节（六维度） | §2（新增） |
| CR5-002 | P0 | （配套见文件 13）矩阵方向修正；本文件补充矩阵方向约定说明 | §3.1（矩阵方向约定） |
| CR5-003 | P1 | （配套见文件 13）depth round-trip 校验；本文件 depth_epsilon_m 记录校准后值 | §3.1（depth_epsilon_m 字段） |
| CR5-004 | P1 | image manifest `v_sun_macro_applied` → `v_sun_macro_mode` + `v_sun_macro_applied_to_image`；一致性检查改比较 `sun_visibility`/`shadow_mapping_method`/`v_sun_macro_mode` | §3.2, §5（source_data）, §8.1 |
| CR5-005 | P1 | 字段名 `visibility_level` → `sun_visibility`，统一为 `null if sun_visibility == "camera_visible_nol"` | §3.1（sun_depth_exr_path, sun_visibility_mask_path 说明） |
| CR5-006 | P2 | `depth_epsilon_m` 改为记录校准后最终值（初始 1e-3 见文件 13 §7.5） | §3.1（depth_epsilon_m 字段说明） |

**附带改进（非 CR5 但一并整改）**：
- image manifest 顶层新增 `sun_visibility` 和 `shadow_mapping_method` 字段，使 §8.1 一致性检查可直接比较（旧版 image manifest 缺这两个字段，CR5-004 要求的比较实际无法执行）。

---

## 十四、提交 Codex 最终短复审清单

**本文件需与 `13_v0.4前向模型冻结规范_最终冻结版.md` 一并提交最终短复审。**

### 14.1 提交文件

```
04_四路线分工区/00_总览与裁决/00_路线冻结文件区/04_BlenderOCS方法重建_全局方法冻结文件/13_v0.4前向模型冻结规范_最终冻结版.md  ← 方法规范
04_四路线分工区/00_总览与裁决/00_路线冻结文件区/04_BlenderOCS方法重建_全局方法冻结文件/14_v0.4数据与manifest字段规范_最终冻结版.md  ← 本文件（数据规范）
```
...
| 优先级 | 章节 | 审阅要点 |
|---|---|---|
| P0 | §2（manifest 立论） | 六维度论证是否回应"为什么不只记录一个 OCS CSV" |
| P1 | §3.2（image manifest） | `v_sun_macro_mode` / `v_sun_macro_applied_to_image` 语义是否清晰、是否与文件 13 一致 |
| P1 | §3.1（字段名） | 是否已全部改为 `sun_visibility == "camera_visible_nol"`，无残留 `visibility_level` |
| P1 | §8.1（一致性检查） | 是否已改为比较 `sun_visibility`/`shadow_mapping_method`/`v_sun_macro_mode` |
| P2 | §3.1（depth_epsilon_m） | 是否明确记录校准后最终值 |
