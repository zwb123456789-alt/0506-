...
> Version: v0.3 Acta Astronautica / Advances in Space Research polished draft  
> Date: 2026-06-05  
...
**Fig. 4 caption intent.** Bidirectional degradation robustness (two panels). Panel (a), image degradation: four curves across image-noise levels, namely clean-trained image-only, clean-trained naive fusion, OCS-only reference at 5.91 deg, and U1 degradation-aware fusion, showing that naive fusion follows image-only collapse while U1 stays stable. Panel (b), OCS degradation: OCS-only versus OCS-image fusion across 0-20% OCS noise (Table 7), with the image-compensation gain rising monotonically. Together the panels show that each modality buffers the other when only one degrades, while the noise tests remain controlled/synthetic degradations rather than a full atmosphere/sensor model.
...
OCS-noise sensitivity also clarifies the conditional value of fusion. Here synthetic relative Gaussian noise is added only to the OCS per-part features while the image branch stays clean, so this is a one-sided modality-degradation analysis rather than a complete field-degradation study. As OCS quality degrades from 0% to 20% noise, OCS-only error rises from 5.91 to 17.25 deg, while clean-image fusion degrades more slowly, so the image-compensation gain increases monotonically from +1.97 deg to +6.29 deg (Table 7, Fig. 4b). The monotonic trend, visible only with the intermediate levels, is what supports the conditional-complementarity reading: the weaker one modality becomes, the more the other contributes. The relative-noise model spans roughly 1% to 20%, consistent with the 0.01 to 0.1 mag relative-flux errors typical of ground-based photometry; a calibrated Poisson/read-noise model is left to future work.

**Table 7. OCS-noise robustness: OCS-only vs OCS-image fusion (image branch clean; relative Gaussian OCS noise; 5 seeds, mean +/- std).**
...
**Fig. 5 caption intent.** Forward-model sensitivity analysis (three panels). Panel (a), self-occlusion: OCS with versus without self-occlusion, with per-geometry mean occlusion of 60.1% to 78.5%, confirming that visibility modeling is not optional. Panel (b), BRDF roughness: OCS sensitivity to +/-20% perturbation of GGX roughness, where metal-body roughness dominates (about 30-46% OCS change) while non-metallic parts vary by less than 5%. Panel (c), roll: OCS variation under roll perturbation (about 20% mean, up to 26% at extreme attitudes), justifying the fixed-roll scope. Phase63 fairness and random-split consistency are reported in Supplementary Figs. S4-S5.
