...
当前状态：定位冻结完成、代码实施前指标规范待冻结
```

本文件是后续代码阶段的资产盘点与实施预案，不是立即开工指令。进入 `06_v0.4_code/` 前，必须先新增并冻结“v0.4 信息量与置信指标实现规范”，把 24 号主线中的 observability、posterior-like distribution、consistency、A/B 口径字段、F1/L1/F2/L2 形态字段、保存字段和最小指标集转成可执行定义。该规范冻结后，本文件才作为代码实施顺序参考。
