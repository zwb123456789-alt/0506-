...
本文件是进入代码阶段后的第一批门控验证清单，不是当前立即执行的第一步。当前下一步仍是先新增并冻结“v0.4 信息量与置信指标实现规范”；该规范冻结并进入 `06_v0.4_code/` 后，再按本文件执行 depth round-trip、20 姿态 shadow validation、V_sun_macro 对图像影响检查等 gate。
