# Step 4 GPT 交互提示词：Method

> 使用对象：GPT  
> 当前任务：在已完成 Introduction 和 Related Work 的基础上，生成论文 Method 章节的结构、英文初稿、方法边界和可复现性说明。  
> 输出建议保存为：`GPT writing/04_Step4_GPT输出_Method.md`
...
- 使用真实卫星 STL 几何。
- 模型包含三个部件：金属主体、太阳能板、遮光板。
- 面元级几何用于 OCS 积分。
- 姿态用 yaw-pitch 表示，roll 在当前 benchmark 中固定。
- 主要姿态网格为 5°：yaw `73` 个采样，pitch `37` 个采样，共 `2701` 个 yaw-pitch attitudes。
- 曾有 10° 网格基线，但正式反演使用 10°→5° split 测试内插泛化。
- 旋转约定：如需写具体矩阵，可写 `[需要作者确认：Euler order / matrix convention]`，不要自行编造。
...
- all_raw / per_part_log / total_log 等可以在 inversion model 小节说明

注意：

- `all_raw 45D` 含遮挡率/额外信息，只能写成 semi-oracle upper bound。
- `per_part_log` 是主要实用 OCS 特征之一。
- 不要在 Method 里提前写结果数值。
...
- ResNet-18 / ResNet image-only as stronger image model for clean synthetic upper-bound evaluation.
...
- yaw is periodic.
- pitch is bounded.
- angular error should account for spherical / yaw-pitch geometry. 如不确定公式细节，用 `[需要作者确认：angular error formula]`。

---

## 4. Method 中必须写清楚的边界

请在 Method 末尾或 Overview 中明确写：
...
写英文 Method 初稿。建议总长 `1600-2400 words`。要求：
...
```text
D:\我的文件\研究生学术\光学项目\0506新\论文改进\论文写作\GPT交互\GPT writing\04_Step4_GPT输出_Method.md
```
